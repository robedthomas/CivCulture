...
namespace CivCulture_Model.Models.MetaComponents.MapGenerations
{
    public class TestMapGeneration : MapGeneration
    {
        ...
        public override GameMap GenerateMap(MapConfiguration config, NamesDatabase namesDb, TemplatesDatabase templatesDb, Random seed, out List<Culture> allCultures, out List<Pop> allPops, out List<Job> allJobs)
        {
            ...
            Job job = new Job(templatesDb.GetJobTemplate("Gatherer_Wilderness"), null); // new BuildingSlot(BuildingSlotTemplate.Wilderness)
            allJobs.Add(job);
            Pop pop = new Pop(templatesDb.GetPopTemplate("Laborer"), null) { Money = 100, Job = job, Space = map.Spaces[0, 0] };
